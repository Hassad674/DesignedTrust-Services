...
describe("useMessagingWS", () => {
  ...
  // ────────────────────────────────────────────────────────────────
  // Regressions for the WS perf protections re-applied after the
  // revert of perf commit c0c68dbf. Mirror of the tests on the
  // global WS hook (use-global-ws): floor 2 s + sentinel sync.
  // ────────────────────────────────────────────────────────────────

  it("reconnect uses 2 s floor on first attempt (no premature retry < 2 s)", async () => {
    // Pin jitter to 0 so we measure the floor only.
    ...
    // 1500 ms < 2 s floor → no new WS yet.
    await act(async () => {
      vi.advanceTimersByTime(1500)
      await Promise.resolve()
    })
  ...
  it("pendingConnect sentinel ensures only one socket is created across rapid re-renders", async () => {
    // If the sentinel is missing, two parallel mounts (StrictMode
    // or rapid rerenders) can both pass the readyState guard during
    // the await getWSUrl() microtask and create two sockets.
    const { rerender } = renderHook(
      ({ uid }: { uid: string }) => useMessagingWS(uid),
      { wrapper: createWrapper(), initialProps: { uid: "user-1" } },
    )

    // Synchronous rerender with the same userId: connect callback
    // identity is stable, but if a future refactor breaks that, the
    // sentinel still prevents a second WS from being created.
    rerender({ uid: "user-1" })
    ...
    // Still CONNECTING — only one socket, never opened.
    expect(mockWSInstance.readyState).toBe(MockWebSocket.CONNECTING)
  })
})
